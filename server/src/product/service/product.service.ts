...
export class ProductService {
  ...
  async uploadProductAI(currentUser, files, productInfo): Promise<boolean> {
    delete productInfo['tag'];
    ...
    const imgUrl = files[0].location;

    console.log('0 productInfo >> ', productInfo);

    await axios
      .post('http://localhost:8005/api/product-ai/info', { imgUrl })
      .then((res) => {
      ...
      .catch((err) => console.log('upload-ai err >> ', err));

    console.log('1 productInfo >> ', productInfo);
